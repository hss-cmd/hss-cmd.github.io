...
[uni-app/offer-notebook](uni-app/offer-notebook/) 是基于 uni-app（Vue 3 + Vite）的真实工程：5 个页面已转写完成，支持编译为微信小程序、H5 与 App（HBuilderX 云打包）。构建与使用说明见[工程 README](uni-app/offer-notebook/README.md)。
...
- 首页：🔍 按公司 / 岗位 / 城市实时搜索；7 个分类导航；点卡片「⋯」可快速改阶段 / 编辑 / 删除；新增投递后卡片高亮并真实加入列表与统计
- 编辑页：完整表单可真实录入并保存展示；AI 截图识别一键填表；编辑旧记录可一键删除
- 发现页：可发帖（自动出现在社区）、评论（评论数实时+1）、点赞、收藏、社区搜索、热榜
- 消息页：消息分类筛选、点消息查看详情并标记已读（角标实时减少）、面试提醒可加入日历
- 个人页：更新简历带真实上传进度、数据导出会真实下载 TXT 投递报告、收藏 / 帖子 / 日历均可查看
- 全部数据保存在浏览器 localStorage，点控制面板「重置演示数据」可一键还原
